...
class Detector:
   ...
   def __init__(self):
      ...
      if detector_type == 'tf':
         ...
         if len(model_dir) == 0:
            raise ValueError('Parameter \'model_dir\' must be passed')
         if len(label_map) == 0:
         ...
         self._detector = TFObjectDetector(model_dir, label_map, confidence=confidence)
         rospy.loginfo('Path to model dir: ' + model_dir)
